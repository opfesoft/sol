...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_INS_IP_AUTO_BANNED, "INSERT INTO ip_banned (ip, bandate, unbandate, bannedby, banreason) VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'AzerothCore realmd', 'Failed login autoban')", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_INS_ACCOUNT_AUTO_BANNED, "INSERT INTO account_banned VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'AzerothCore realmd', 'Failed login autoban', 1)", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_SEL_LOGONCHALLENGE, "SELECT a.id, a.locked, a.lock_country, a.last_ip, aa.gmlevel, a.v, a.s, a.token_key FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.username = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_INS_ACCOUNT, "INSERT INTO account(username, expansion, joindate) VALUES(?, ?, NOW())", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_SEL_CHECK_PASSWORD, "SELECT v, s FROM account WHERE username = ?", CONNECTION_SYNCH);
