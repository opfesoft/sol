...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_UPD_WAYPOINT_DATA_POINT_SHIFT_UP, "UPDATE waypoint_data SET point = point + 1 WHERE id = ? AND point > ? ORDER BY point DESC", CONNECTION_ASYNC);
