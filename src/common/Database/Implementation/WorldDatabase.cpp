...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_UPD_WAYPOINT_DATA_POINT_SHIFT_DOWN, "UPDATE waypoint_data SET point = point - 1 WHERE id = ? AND point > ? ORDER BY point ASC", CONNECTION_ASYNC);
    ...
    PrepareStatement(WORLD_SEL_WAYPOINT_DATA_ALL_BY_WPGUID, "SELECT id, point, delay, move_type, action, action_chance, orientation FROM waypoint_data WHERE wpguid = ?", CONNECTION_SYNCH);
