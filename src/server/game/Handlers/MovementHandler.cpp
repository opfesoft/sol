...
void WorldSession::HandleMovementOpcodes(WorldPacket & recvData)
{
    ...
    {
        sLog->outString("The computed movement time using clockDelta is erronous. Using fallback instead");
    ...
    if (plrMover)                                            // nothing is charmed, or player charmed
    {
        ...
        if (movementInfo.pos.GetPositionZ() < plrMover->GetMap()->GetMinHeight(movementInfo.pos.GetPositionX(), movementInfo.pos.GetPositionY()))
            if (!plrMover->GetBattleground() || !plrMover->GetBattleground()->HandlePlayerUnderMap(_player))
            {
                ...
                {
                    GraveyardStruct const* grave = sGraveyard->GetClosestGraveyard(plrMover->GetPositionX(), plrMover->GetPositionY(), plrMover->GetPositionZ(), plrMover->GetMapId(), plrMover->GetTeamId(), plrMover->getClass());
                    if (grave)
                    {
...
void WorldSession::HandleTimeSyncResp(WorldPacket& recvData)
{
    ...
    uint32 roundTripDuration = getMSTimeDiff(serverTimeAtSent, recvData.GetReceivedTime().msec());
