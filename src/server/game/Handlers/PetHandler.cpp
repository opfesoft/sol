...
uint8 WorldSession::HandleLoadPetFromDBFirstCallback(PreparedQueryResult result, uint8 asynchLoadType)
{
    ...
    float px, py, pz;
    owner->GetNearPoint(pet, px, py, pz, pet->GetObjectSize(), PET_FOLLOW_DIST, owner->GetOrientation() + pet->GetFollowAngle());
    ...
    if (pet->getPetType() == HUNTER_PET || pet->GetCreatureTemplate()->type == CREATURE_TYPE_DEMON || pet->GetCreatureTemplate()->type == CREATURE_TYPE_UNDEAD)
        pet->GetCharmInfo()->SetPetNumber(pet_number, pet->IsPermanentPetFor(owner)); // Show pet details tab (Shift+P) only for hunter pets, demons (Warlock) or undead (Death knight)
    else
    ...
    if (summon_spell_id && spellInfo && (spellInfo->CategoryRecoveryTime > 0 || spellInfo->RecoveryTime > 0))
    {
