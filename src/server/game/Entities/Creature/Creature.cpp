...
m_transportCheckTimer(1000), lootPickPocketRestoreTime(0), m_wpActiveTimer(0), m_wpActiveTimerInit(0), m_wpInactiveTimer(0), m_wpInactiveTimerInit(0), m_wpScriptTimer(0),
m_reactState(REACT_AGGRESSIVE), m_storedReactState(REACT_AGGRESSIVE), m_defaultMovementType(IDLE_MOTION_TYPE),
...
m_spawnedByDefault(true), m_playerDamageReq(0), m_damagedByPlayer(false), m_hasLevelRange(false), m_saveRespawnTime(true), m_updateHealth(false), m_maxAggroRadius(-1.f), m_randomGender(true), m_leaderMoveFailed(false)
{
...
void Creature::LoadPath(uint32 pathid)
{
    ...
    uint32 movActiveTimer = sWorld->getIntConfig(CONFIG_WAYPOINT_MOVEMENT_ACTIVE_TIMER);
    if (!movActive || !movActiveTimer)
        return; // never active

    if (pathid)
    {
        movActive *= 1000;
        m_wpInactiveTimerInit = urand(movActive - movActive / 4, movActive + movActive / 4);
        m_wpInactiveTimer = m_wpInactiveTimerInit;

        movActiveTimer *= 1000;
        m_wpActiveTimerInit = urand(movActiveTimer - movActiveTimer / 4, movActiveTimer + movActiveTimer / 4);
        GetMap()->AddToCreaturesWP(this);
    }
    else
    {
        m_wpInactiveTimerInit = 0;
        m_wpActiveTimerInit = 0;
...
{ 
    ...
    if (cInfo->flags_extra & CREATURE_FLAG_EXTRA_NO_MOVE_FLAGS_UPDATE)
...
{ 
    ...
    if (fleeAssistRadius > 0)
    {
        ...
        {
            // check if the target creature is actually reachable
...
{ 
    SetZoneScript();
    if (!(sObjectMgr->GetCreatureIdChanceVector(m_DBTableGuid) || sObjectMgr->GetCreatureGuidChanceId(m_DBTableGuid)))
    {
...
{ 
    ...
    if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC) && enemy->GetTypeId() == TYPEID_PLAYER)
        return false;
...
{ 
    if (!m_saveRespawnTime || IsSummon() || !GetDBTableGUIDLow() || (m_creatureData && !m_creatureData->dbData) || !m_spawnedByDefault)
        return;
...
uint32 Creature::UpdateWPInactiveTimer(uint32 diff)
{
    uint32 oldValue = m_wpInactiveTimer;

    if (m_wpInactiveTimer <= diff)
        m_wpInactiveTimer = 0;
    else
        m_wpInactiveTimer -= diff;

    return oldValue;
}

void Creature::SetWPScriptTimer(uint32 wpScriptTime)
{
    if (m_wpScriptTimer < wpScriptTime)
        m_wpScriptTimer = wpScriptTime;
}

void Creature::UpdateWPScriptTimer(uint32 diff)
{
    if (m_wpScriptTimer <= diff)
        m_wpScriptTimer = 0;
    else
        m_wpScriptTimer -= diff;
}

void Creature::DelayKill(Unit* killer, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const* spellProto)
{
    KillDelayEvent* event = new KillDelayEvent(killer ? killer->GetGUID() : 0, *this, durabilityLoss, attackType, spellProto);
    m_killTime = m_Events.CalculateTime(m_killDelay);
    m_Events.AddEvent(event, m_killTime);
    m_killDelay = 0;
}

bool KillDelayEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    if (Creature* creature = m_owner.ToCreature())
    {
        creature->SetKillDelay(0);
        creature->SetKillTime(0);
    }

    Unit::Kill(m_killer ? ObjectAccessor::GetUnit(m_owner, m_killer) : NULL, &m_owner, m_durabilityLoss, m_attackType, m_spellProto);
    return true;
}

void Creature::SendHealthUpdate()
{
    WorldPacket data(SMSG_HEALTH_UPDATE, 16 + 4);
    data.append(GetPackGUID());
    data << int32(GetHealth());
    SendMessageToSet(&data, true);
}
