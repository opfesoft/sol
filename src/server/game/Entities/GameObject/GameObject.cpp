...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_CHAIR:                         //7
        {
            ...
            // a chair may have n slots. we have to calculate their positions and teleport the user to the nearest one
            ...
            for (ChairSlotAndUser::iterator itr = ChairListSlots.begin(); itr != ChairListSlots.end(); ++itr)
            {
                ...
                if (itr->second)
                {
                    if (Unit* ChairUser = ObjectAccessor::GetUnit(*this, itr->second))
                    {
            ...
            if (found_free_slot)
            {
                ...
                {
                    itr->second = user->GetGUID(); //this slot in now used by user
...
{ 
    ...
    float rotation = 2.0f * M_PI - GetOrientation();
    float sinVal = std::sin(rotation);
    float cosVal = std::cos(rotation);
    ...
    float rotX = float(GetPositionX() + dx * cosVal - dy * sinVal);
    float rotY = float(GetPositionY() + dy * cosVal + dx * sinVal);

    dx = rotX - GetPositionX();
    dy = rotY - GetPositionY();
