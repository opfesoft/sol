...
ProcEventInfo::ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget, uint32 typeMask, uint32 spellTypeMask, uint32 spellPhaseMask, uint32 hitMask, Spell const* spell, DamageInfo* damageInfo, HealInfo* healInfo, SpellInfo const* triggeredByAuraSpell)
...
i_motionMaster(new MotionMaster(this)), m_regenTimer(0), m_ThreatManager(this), m_vehicle(NULL), m_vehicleKit(NULL), m_unitTypeMask(UNIT_MASK_NONE), m_splineSyncTimer(0), m_HostileRefManager(this)
{
...
void Unit::Update(uint32 p_time)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && HasUnitState(UNIT_STATE_CASTING))
        for (Spell* spell : m_currentSpells)
            if (spell && spell->GetSpellInfo()->HasAttribute(SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS))
            {
    ...
    if (int32 ranged_attack = getAttackTimer(RANGED_ATTACK); ranged_attack && !suspendRangedAttackTimer)
...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool  /*allowGM*/)
{
    ...
    // Don't deal damage to player if "cheat god" is enabled
...
SpellCastResult  Unit::CastSpell(Unit* victim, SpellInfo const* spellInfo, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    ...
    return CastSpell(targets, spellInfo, NULL, triggerFlags, castItem, triggeredByAura, originalCaster);
}
...
SpellCastResult Unit::CastSpell(float x, float y, float z, uint32 spellId, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    ...
    return CastSpell(targets, spellInfo, NULL, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura, originalCaster);
}
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    data << uint32(victim ? victim->isDead() : 0);
    SendMessageToSet(&data, true);
#if defined(ENABLE_EXTRAS) && defined(ENABLE_EXTRA_LOGS)
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (!victim->HasInArc(M_PI, this) || spell->HasAttribute(SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET))
    {
        ...
        {
            if (spell->HasAttribute(SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET))
                canParry = false;
        }
    }
...
bool Unit::CanMoveDuringChannel() const
{
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL])
        if (spell->getState() != SPELL_STATE_FINISHED)
            return (spell->GetSpellInfo()->HasAttribute(SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING) || !(spell->GetSpellInfo()->ChannelInterruptFlags & (AURA_INTERRUPT_FLAG_MOVE | AURA_INTERRUPT_FLAG_TURNING))) && spell->IsChannelActive();
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, uint64 casterGUID /*= 0*/, bool periodicReset /*= false*/)
{
    ...
    if (!newAura->IsMultiSlotAura())
    {
        ...
        if (Aura* foundAura = GetOwnedAura(newAura->Id, newAura->HasAttribute(SPELL_ATTR0_CU_SINGLE_AURA_STACK) ? 0 : casterGUID, (!newAura->HasAttribute(SPELL_ATTR0_CU_SINGLE_AURA_STACK) && newAura->HasAttribute(SPELL_ATTR0_CU_ENCHANT_PROC)) ? castItemGUID : 0, 0))
        {
...
void Unit::RemoveEvadeAuras()
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (aura->GetSpellInfo()->HasAttribute(SPELL_ATTR1_AURA_STAYS_AFTER_COMBAT) || aura->GetSpellInfo()->HasAura(SPELL_AURA_CONTROL_VEHICLE) || aura->GetSpellInfo()->HasAura(SPELL_AURA_CLONE_CASTER) || (aura->IsPassive() && IS_PLAYER_GUID(GetOwnerGUID())))
            ++iter;
        else
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
    }
...
void Unit::SendSpellNonMeleeReflectLog(SpellNonMeleeDamage* log, Unit* attacker)
{
    ...
    // If "cheat god" is enabled swap absorb with damage and set damage to 0; this way you can still debug damage but won't loose health
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                ...
                {
                    // mana back
                    basepoints0 = int32(CalculatePct(spellProc ? spellProc->GetPowerCost() : 0, 30));
                    target = this;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (dummySpell->SpellIconID == 2018)            // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
                ...
                if (!spellProc || (spellProc && spellProc->GetFirstUniqueTargetGUID() != victim->GetGUID()))
                    return false;
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (trigger_spell_id)
    {
        ...
        case 40431: // Curse of Mending
        case 60782: // Curse of Mending
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && !(IsControllableGuardian() && IS_PLAYER_GUID(GetOwnerGUID())))
    {
...
float Unit::SpellPctDamageModsDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype)
{
    ...
    for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
    {
        ...
        if (spellProto->EquippedItemClass == -1 && (*i)->GetSpellInfo()->EquippedItemClass != -1 &&
            !(*i)->GetSpellInfo()->HasAttribute(SPELL_ATTR5_SPECIAL_ITEM_CLASS_CHECK) && (*i)->GetMiscValue() == SPELL_SCHOOL_MASK_NORMAL)
            continue;
...
void Unit::AddThreat(Unit* victim, float fThreat, SpellSchoolMask schoolMask, SpellInfo const* threatSpell)
{
    ...
    if (CanHaveThreatList() && !HasUnitState(UNIT_STATE_EVADE))
    {
        ...
        {
            AddThreat(vehicle, fThreat, schoolMask, threatSpell); // Threat generated by passengers is added to the vehicle instead
            if (victim->HasUnitTypeMask(UNIT_MASK_ACCESSORY)) // Accessories are part of the vehicle and cannot have threat
                return;
            fThreat = 0.f;
        }
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, int32 effectiveValue, SpellInfo const* procAura, Spell const* spell)
{
    ...
    ProcEventInfo eventInfo = ProcEventInfo(actor, actionTarget, target, procFlag, 0, 0, procExtra, spell, &damageInfo, &healInfo, procAura);
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent, ProcEventInfo const& eventInfo)
{
    ...
    if (!sSpellMgr->IsSpellProcEventCanTriggeredBy(spellProto, spellProcEvent, EventProcFlag, procSpell, procFlag, procExtra, active, eventInfo.GetSpell()))
        return false;
...
void Unit::Kill(Unit* killer, Unit* victim, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const *spellProto)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->getClass() == CLASS_PRIEST && !victim->ToPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_IS_OUT_OF_BOUNDS))
    {
        ...
        switch (state)
        {
            ...
            case UNIT_STATE_ROOT:
                if (Creature* c = ToCreature())
                    if (CreatureAI* ai = c->AI(); ai && ai->IsForceRoot())
                        return;
