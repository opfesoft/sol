...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Mask_CatmullRom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            if (fabs(offset.x) >= static_cast<float>(MAX_OFFSET) || fabs(offset.y) >= static_cast<float>(MAX_OFFSET) || fabs(offset.z) >= static_cast<float>(MAX_OFFSET))
            {
        ...
        else
        {
            if (spline.isCyclic())
            {
                ...
                if (splineflags.enter_cycle)
                {
                    ...
                    MoveSplineInitArgs args{ (size_t)spline.getPointCount(true) };
                    args.path.assign(spline.getPoints(true).begin() + spline.first() + 1, spline.getPoints(true).begin() + spline.last());
                    args.facing = facing;
                    ...
                    if (args.Validate(nullptr))
                    {
                        ...
                        Movement::SplineBase::ControlArray* visualPoints = const_cast<Movement::SplineBase::ControlArray*>(spline.allocateVisualPoints());
                        visualPoints->resize(spline.getPointCount(false));
                        std::copy(spline.getPoints(false).begin(), spline.getPoints(false).end(), visualPoints->begin());
                    }
                }
            }
            else
