...
enum ProcFlagsExLegacy
{
    ...
    PROC_EX_ONLY_ON_PLAYER_PRE_CAST = 0x0004000,                 // If set trigger only once on player cast (before the actual cast)
    ...
    PROC_EX_ONLY_ON_PLAYER_CAST     = 0x0020000,                 // If set trigger only once on player cast (after the actual cast)
...
class SpellMgr
{
    ...
    public:
        ...
        bool IsSpellProcEventCanTriggeredBy(SpellInfo const* spellProto, SpellProcEventEntry const* spellProcEvent, uint32 EventProcFlag, SpellInfo const* procSpell, uint32 procFlags, uint32 procExtra, bool active, Spell const* spell) const;
