...
void Spell::SelectImplicitTrajTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    std::list<WorldObject*> targets;
    acore::WorldObjectSpellTrajTargetCheck check(dist2d, &srcPos, m_caster, m_spellInfo, TARGET_CHECK_ENEMY /*targetCheckType*/, m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
    ...
    for (; itr != targets.end(); ++itr)
    {
        ...
        if (m_CastItem)
        {
            if (distToHitPoint < bestDist && fabs(dz) < 6.0f) // closest target, also check Z difference
            {
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (((m_spellFlags & SPELL_FLAG_REFLECTED) && !(unit->HasAuraType(SPELL_AURA_REFLECT_SPELLS))) || (aura_effmask && m_diminishGroup && unit != m_caster && (m_caster->GetTypeId() != TYPEID_UNIT || !m_caster->ToCreature()->isWorldBoss())))
    {
        ...
        if ((type == DRTYPE_PLAYER && (
            unit->IsCharmedOwnedByPlayerOrPlayer() || flagsExtra & CREATURE_FLAG_EXTRA_ALL_DIMINISH || (m_diminishGroup == DIMINISHING_TAUNT && (flagsExtra & CREATURE_FLAG_EXTRA_TAUNT_DIMINISH))
            )) || type == DRTYPE_ALL)
            if (m_caster->IsCharmedOwnedByPlayerOrPlayer()) // Do not apply diminish return if caster is NPC
    ...
    if (m_caster != unit && m_caster->IsHostileTo(unit) && !m_spellInfo->IsPositive() && !m_triggeredByAuraSpell && m_spellInfo->SpellIconID != 2267 && unit->IsControlledByPlayer() && (!m_caster->IsTotem() || m_spellInfo->Id == 64695) && /*Intervene Trigger*/ m_spellInfo->Id != 59667 && /*Soulshatter*/ m_spellInfo->Id != 29858 && /*Premeditation*/ m_spellInfo->Id != 14183)
    ...
    if (aura_effmask)
    {
        ...
        if (m_originalCaster)
        {
            ...
            m_spellAura = Aura::TryRefreshStackOrCreate(aurSpellInfo, effectMask, unit, m_originalCaster, 
                (aurSpellInfo == m_spellInfo)? &m_spellValue->EffectBasePoints[0] : &basePoints[0], m_CastItem, 0, m_caster->GetGUID(), &refresh, !(_triggeredCastFlags & TRIGGERED_NO_PERIODIC_RESET));
...
SpellCastResult Spell::CheckRuneCost(uint32 runeCostID)
{
    ...
    // If "cheat power" is enabled don't check the cost as you're able to use the spell anyways (infinite power)
...
SpellCastResult Spell::CheckPower()
{
    ...
    // While "cheat power" is enabled don't check if power is needed to cast the spell
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
        if (m_caster->ToPlayer()->GetCommandStatus(CHEAT_POWER))
            return SPELL_CAST_OK;
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CREATE_ITEM_2:
            {
                ...
                Unit* target = m_targets.GetUnitTarget() ? m_targets.GetUnitTarget() : p_caster;
                if (target->GetTypeId() == TYPEID_PLAYER && !IsTriggered())
                {
                    ...
                    if (m_spellInfo->Effects[i].Effect == SPELL_EFFECT_CREATE_ITEM_2)
                        ...
                        {
                            p_caster->SendEquipError(EQUIP_ERR_INVENTORY_FULL, nullptr, nullptr, m_spellInfo->Effects[i].ItemType);
                            return SPELL_FAILED_DONT_REPORT;
                        }
                        ...
                        if (msg != EQUIP_ERR_OK)
                        {
                            ...
                            else
                            {
                                ...
                                else if (!(target->ToPlayer()->HasItemCount(m_spellInfo->Effects[i].ItemType)))
                                {
                                    ...
                                    p_caster->CastSpell(p_caster, m_spellInfo->Effects[EFFECT_1].CalcValue(), false);        // move this to anywhere
                                return SPELL_FAILED_DONT_REPORT;
                            }
                        }
                    }
            ...
            {
                if (p_caster->GetFreeInventorySpace() == 0)
                {
                    p_caster->SendEquipError(EQUIP_ERR_INVENTORY_FULL, nullptr, nullptr, m_spellInfo->Effects[i].ItemType);
                    return SPELL_FAILED_DONT_REPORT;
                }
...
void Spell::TriggerGlobalCooldown()
{
    ...
    if (m_spellInfo->StartRecoveryTime >= MIN_GCD && m_spellInfo->StartRecoveryTime <= MAX_GCD)
    {
        ...
        if (m_spellInfo->StartRecoveryCategory == 133 && m_spellInfo->StartRecoveryTime == 1500 && m_spellInfo->DmgClass != SPELL_DAMAGE_CLASS_MELEE &&
            m_spellInfo->DmgClass != SPELL_DAMAGE_CLASS_RANGED && !m_spellInfo->HasAttribute(SPELL_ATTR0_REQ_AMMO) && !m_spellInfo->HasAttribute(SPELL_ATTR0_ABILITY))
...
bool WorldObjectSpellTrajTargetCheck::operator()(WorldObject* target)
{
    ...
    if (!_caster->HasInLine(target, std::max(target->GetObjectSize() * 0.7f, 1.0f) + TRAJECTORY_MISSILE_SIZE))
        return false;
    return WorldObjectSpellAreaTargetCheck::operator ()(target);
}
