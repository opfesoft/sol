...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleModAuraVision,                             //170 SPELL_AURA_MOD_AURA_VISION       used to detect various spells that change visual of units for aura target
...
void AuraEffect::HandleModAuraVision(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    else if (!target->HasAuraTypeWithMiscvalue(SPELL_AURA_DETECT_AMORE, GetMiscValue()))
...
void AuraEffect::HandleAuraModStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        sLog->outError("Spell %u effect %u has an unsupported misc value (%i) for SPELL_AURA_MOD_STAT ", GetId(), GetEffIndex(), GetMiscValue());
        return;
    }
...
void AuraEffect::HandleModPercentStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        sLog->outError("Misc Value for SPELL_AURA_MOD_PERCENT_STAT not valid");
        return;
    }
...
void AuraEffect::HandleAuraRetainComboPoints(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (!(apply) && GetBase()->GetDuration() == 0 && target->GetComboTarget())
        if (Unit* unit = ObjectAccessor::GetUnit(*target, target->GetComboTarget()))
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    caster->ProcDamageAndSpell(target, caster ? procAttacker : 0, procVictim, procEx, damage, damage - overkill, BASE_ATTACK, GetSpellInfo());
}
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    // allow null caster to call this function
    caster->ProcDamageAndSpell(target, caster ? procAttacker : 0, procVictim, procEx, damage, damage, BASE_ATTACK, GetSpellInfo());
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (!haveCastItem && GetAuraType() != SPELL_AURA_OBS_MOD_HEALTH) // xinef: dont allow obs_mod_health to proc spells, this is passive regeneration and not hot
        ...
        caster->ProcDamageAndSpell(target, caster ? procAttacker : 0, procVictim, procEx, damage, gain, BASE_ATTACK, GetSpellInfo());
}
...
void AuraEffect::HandlePeriodicPowerBurnAuraTick(Unit* target, Unit* caster) const
{
    ...
    int32 effectiveDamage = damageInfo.damage < target->GetHealth() ? damageInfo.damage : target->GetHealth();
    caster->ProcDamageAndSpell(damageInfo.target, procAttacker, procVictim, procEx, damageInfo.damage, effectiveDamage, BASE_ATTACK, spellProto);
}
