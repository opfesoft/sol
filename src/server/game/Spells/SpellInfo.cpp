...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* /*target*/) const
{
    ...
    if (caster)
    {
        ...
        if (caster->m_movedByPlayer)
            if (uint8 comboPoints = caster->m_movedByPlayer->GetComboPoints())
...
AuraStateType SpellInfo::LoadAuraState() const
{
    ...
    uint32 StealthPreventionSpellList[] = { 9991, 35331, 9806, 35325, 16498, 6950, 20656, 25602, 32129, 35328, 35329 };
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell) const
{
    ...
    powerCost += caster->GetInt32Value(static_cast<int>(UNIT_FIELD_POWER_COST_MODIFIER) + static_cast<int>(school));
    ...
    powerCost = int32(powerCost * (1.0f + caster->GetFloatValue(static_cast<int>(UNIT_FIELD_POWER_COST_MULTIPLIER) + static_cast<int>(school))));
    if (powerCost < 0)
        powerCost = 0;
    return powerCost;
}
