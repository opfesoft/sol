    ...
    namespace XP
    {
        ...
        inline uint32 Gain(Player* player, Unit* u, bool isBattleGround = false)
        {
                ...
                !(creature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_XP_AT_KILL)))
            {
                ...
                if (creature && creature->GetPlayerDamageReq()) // if players dealt less than 50% of the damage and were credited anyway (due to CREATURE_FLAG_EXTRA_NO_PLAYER_DAMAGE_REQ), scale XP gained appropriately (linear scaling)
