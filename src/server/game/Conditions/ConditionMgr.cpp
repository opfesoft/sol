...
bool Condition::Meets(ConditionSourceInfo& sourceInfo, bool conditionTargetNullResult /*= false*/)
{
    ...
    switch (ConditionType)
    {
        ...
        {
            if (GameObject* go = GetClosestGameObjectWithEntry(object, ConditionValue1, (float)ConditionValue2, ConditionValue3 > 0))
                if (ConditionValue3 <= 1 || (ConditionValue3 > 1 && go->GetGoState() == (GOState)(ConditionValue3 - 2)))
                    condMeets = true;
            break;
        }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_QUESTSTATE:
        {
            ...
            {
                sLog->outErrorDb("ConditionType (%u) quest (%u) has invalid state mask (%u), skipped.", cond->ConditionType, cond->ConditionValue1, cond->ConditionValue2);
                return false;
            }
            [[fallthrough]];
        }
        ...
        case CONDITION_QUEST_SATISFY_EXCLUSIVE:
        {
            ...
            {
                sLog->outErrorDb("ConditionType (%u) specifies non-existing quest (%u), skipped", cond->ConditionType, cond->ConditionValue1);
                return false;
            }
            break;
        }
        ...
        case CONDITION_NEAR_GAMEOBJECT:
        {
            ...
            if (cond->ConditionValue3 > MAX_GO_STATE + 1)
            {
                sLog->outErrorDb("NearGameObject condition value3 > %u (%u), skipped", MAX_GO_STATE + 1, cond->ConditionValue3);
                return false;
            }
            break;
        }
