        ...
        void HandleEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (ar && ar->ToPlayer())
            {
                if (ar->ToPlayer()->GetQuestStatus(11317) == QUEST_STATUS_INCOMPLETE || ar->ToPlayer()->GetQuestStatus(11322) == QUEST_STATUS_INCOMPLETE)
                    if (Creature* c = ar->SummonCreature(27959, 3032.0f, -5095.0f, 723.0f, 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000))
                    {
                        c->setActive(true);
                ...
                if (GetCastItem())
                    if (Creature* creatureTarget = GetHitCreature())
                    {
                        ...
                        if (newEntry)
                        {
                            ...
                            creatureTarget->setActive(true);
                            creatureTarget->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                            creatureTarget->GetMotionMaster()->MoveFleeing(creatureTarget);
