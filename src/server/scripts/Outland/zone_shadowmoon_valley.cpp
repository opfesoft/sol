...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* /*creature*/, uint32 /*sender*/, uint32 action) override
    {
        ...
        if (action == GOSSIP_ACTION_INFO_DEF+1)
        {
            ...
            uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, ITEM_FLAUNISS_PACK, 1, NULL);
            if (msg == EQUIP_ERR_OK)
        ...
        {
            ItemPosCountVec dest;
            uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, ITEM_KAGROSHS_PACK, 1, NULL);
            if (msg == EQUIP_ERR_OK)
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        else if (player->GetQuestStatus(QUEST_THE_FATE_OF_KAGROSH) == QUEST_STATUS_INCOMPLETE && !player->HasItemCount(ITEM_KAGROSHS_PACK, 1, true))
