...
public:
    ...
    static bool HandleNpcAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (Transport* tt = chr->GetTransport())
            ...
            {
                uint32 guid = sObjectMgr->GenerateReservedLowGuid(HIGHGUID_UNIT);
        ...
        if (!creature->Create(sObjectMgr->GenerateReservedLowGuid(HIGHGUID_UNIT), map, chr->GetPhaseMaskForSpawn(), id, 0, x, y, z, o))
        {
            delete creature;
            return false;
        }
